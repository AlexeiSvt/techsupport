...
// DevicresCalculator analyzes the history of devices associated with a use.
...
func (c *DevicesCalculator) Calculate(ctx context.Context, user models.UserData, db models.DBRecord, weights models.Weights) models.CalcResult {
	...
	res := c.checkAllDevices(ctx, user.UserClaim.Devices, db.Devices, db.RegDate)
