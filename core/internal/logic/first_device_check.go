...
func (c *FirstDeviceCalculator) Calculate(ctx context.Context, claim models.UserClaim, support models.SupportContext, db models.DBRecord, weights models.Weights) models.CalcResult {
    ...
    // Access flat fields from the updated UserClaim and DBRecord models.
    userDev := claim.FirstDeviceName
    dbDev := db.FirstDevice

    if logger != nil {
        // Use UBTicketID to maintain tracing consistency across bot sessions.
        logger.Debugw("checking first device match", 
            "user_device", userDev, 
            "db_device", dbDev,
...
func (c *FirstDeviceCalculator) checkFirstDevice(ctx context.Context, userDev, dbDev string) rawCheckResult {
    ...
    if u == "" || d == "" {
        // Logging for empty data is now handled at the higher Calculate level to reduce lock contention.
